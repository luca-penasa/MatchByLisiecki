...
// needed for C-like memmove and realloc
...
struct floatlistkeys{
  string key;
  vector<floatnan> *value;
};
struct strlistkeys{
  string key;
  vector<string> *value;
...
#define DEFINE_KEYS floatkeys fk[]={{"nomatch",&nomatch},\
			...
			{"numintervals2", &cnt2},\
			{"",0}};\
       floatnankeys fnk[]={{"normbegin1",&normst1},\
			   {"normbegin2",&normst2},\
			   {"normend1",&normend1},\
			   {"normend2",&normend2},\
			   {"",0}};\
       floatlistkeys flk[]={{"mean1",&mean1},\
			    {"mean2",&mean2},\
			    {"stddev1",&stddev1},\
			    {"stddev2",&stddev2},\
			    {"",0}};\
...
bool config::read(string file){
  ...
  while(!is.eof() && !is.fail()) {
	...
	if (fnk[i].key==key){
	  ...
	  floatnan fvalue;
	  try {
	    fvalue=floatnan(value,0);
	  } catch (floatnan::parse_error) {
	    cerr<<"Value for '" << key << "' is bad or missing.\n";
	    return false;
	  }
	  (*fnk[i].value)=fvalue;
  ...
  if(mean1.size()!=series1.size() && mean1.size()!=0) {
    cerr<<"The number of means must match the number of series1 files.\n";
    result=false;
  }
  if(mean2.size()!=series2.size() && mean2.size()!=0) {
    cerr<<"The number of means must match the number of series2 files.\n";
    result=false;
  }
  if(stddev1.size()!=series1.size() && stddev1.size()!=0) {
    cerr<<"The number of stddevs must match the number of series1 files.\n";
    result=false;
  }
  if(stddev2.size()!=series2.size() && stddev2.size()!=0) {
    cerr<<"The number of stddevs must match the number of series2 files.\n";
    result=false;
  }
  if(nspeeds==0) {
...
void config::write(ostream &os){
  ...
  for(int i=0; fnk[i].value; i++){
    os<<fnk[i].key<<" "<<*fnk[i].value<<"\n";
  }
  ...
  for(int i=0; flk[i].value; i++){
    if(flk[i].value->size()!=0){
      os<<flk[i].key;
      for(unsigned int k=0; k<flk[i].value->size(); k++) 
	os<<" "<<((*flk[i].value)[k]);
      os<<"\n";
    } else{
      os<<"#"<<flk[i].key<<" is not set.\n";
    }
  }
