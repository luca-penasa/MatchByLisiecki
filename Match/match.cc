...
ostream &operator<<(ostream &s, const penaltycomponents &pcomp) {
  ...
  s.precision(3);
  s<<"---Penalties---"<<endl;
  s<<"      point: "<<setw(12)<<pcomp.point<<endl;
  s<<"    nomatch: "<<setw(12)<<pcomp.no_match<<endl;
  s<<"      speed: "<<setw(12)<<pcomp.speed<<endl;
  s<<"speedchange: "<<setw(12)<<pcomp.speed_change<<endl;
  s<<"        tie: "<<setw(12)<<pcomp.tie<<endl;
  s<<"        gap: "<<setw(12)<<pcomp.gap<<endl;
  s<<"tie nomatch: "<<setw(12)<<pcomp.tie_no_match<<endl;
  s<<"-------------------------"<<endl;
  s<<"      Total: "<<setw(12)<<pcomp.total<<endl;
  s.precision(0);
...
series getseries(ostream &log, vector<string> &conf_series, 
		 ...
		 floatnan normst, floatnan normend, 
		 vector<floatnan> mean, vector<floatnan> stddev,
	         string gapfile) {
  if(normst.my_isnan()) normst=start;
  if(normend.my_isnan()) normend=end;

  vector<series> sv;
  for(unsigned int sig=0; sig<conf_series.size(); sig++) {
    sv.push_back(readfile(conf_series[sig]));
    floatnan smean, sstddev;
    if(mean.size()) smean=mean[sig];
    if(stddev.size()) sstddev=stddev[sig];
    if(!smean.my_isnan() || !sstddev.my_isnan()) {
      if(!normalize(sv.back(), 0, normst, normend, smean, sstddev)) {
	cerr<<"ERROR: Too few points or stddev=0 in series "
	    <<conf_series[sig]<<endl;
	log<<"ERROR: Too few points or stddev=0 in series "
	   <<conf_series[sig]<<endl;
	exit(1);
      }
    }
...
int main(int argc, char* argv[]) {
  ...
  s1=getseries(log,conf.series1,"series1",
	       ...
	       conf.normst1, conf.normend1, conf.mean1, conf.stddev1,
	       conf.s1gapfile);
  s2=getseries(log,conf.series2,"series2",
	       conf.st2,conf.end2,int(conf.cnt2),
	       conf.normst2, conf.normend2, conf.mean2, conf.stddev2,
	       conf.s2gapfile);
